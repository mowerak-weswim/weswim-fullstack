...
const html = fs.readFileSync(path.join(pubDir, "record-detail.html"), "utf8");
let css = html.match(/<style>([\s\S]*?)<\/style>/)[1];
css = css.replace(/:root\s*\{[\s\S]*?\}\s*/m, "");
css = css.replace(/html,body\{[^}]*\}/, "");
css = css.replace(/body\{[^}]*\}/, "");

const wrapper = "record-detail-screen";
...
const scoped = vars + scopeCss(css, prefix);
fs.writeFileSync("src/styles/weswim-record-detail.css", scoped);
