...
const html = fs.readFileSync(
  "reference/publishing/screens/group-find.html",
  "utf8",
);
...
function scopeCss(input) {
  ...
  while (i < input.length) {
    ...
    if (sel && !sel.startsWith(prefix.trim())) {
      const parts = sel.split(",").map((s) => {
        const t = s.trim();
        if (!t || t.startsWith(".group-find-page")) {
          return t;
        }
        return prefix + t;
      });
...
const vars = `.group-find-page {
  ...
  --gray-50:#F5F7FA; --gray-100:#ECEFF4; --gray-200:#D0D7E2;
  --gray-300:#B4BECD; --gray-500:#6B7A99; --gray-700:#3D4A66;
  --text-main:#1A1A2E;
...
const scoped = vars + scopeCss(css);
fs.writeFileSync("src/styles/weswim-group-find.css", scoped);
